// Modified version of https://github.com/oktal/result
//
//
//
//                               Apache License
...
namespace fx::result {
  ...
  types::Err<CleanE> Error(E&& value) {
  ...
  struct Storage {
    ...
    void construct(types::Ok<T> ok) {
      new (&_storage) T(ok.value);
      _initialized = true;
    }
    void construct(types::Err<E> err) {
      new (&_storage) E(err.value);
      _initialized = true;
    }

    template <typename U>
    void rawConstruct(U&& value) {
      typedef typename std::decay<U>::type CleanU;

      new (&_storage) CleanU(std::forward<U>(value));
      _initialized = true;
    }
  ...
  struct Result {
    ...
    Result(types::Ok<T> ok) : _ok(true) {
      _storage.construct(std::move(ok));
    }

    Result(types::Err<E> err) : _ok(false) {
      _storage.construct(std::move(err));
    }

    ~Result() {
      if (_ok) {
        _storage.destroy(ok_tag());
      } else {
        _storage.destroy(err_tag());
      }
    }

    Result(Result&& other) {
      if (other.ok()) {
        Constructor<T, E>::move(std::move(other._storage), _storage, ok_tag());
        _ok = true;
      } else {
        Constructor<T, E>::move(std::move(other._storage), _storage, err_tag());
        _ok = false;
      }
    }
    ...
    Result(const Result& other) {
      if (other.ok()) {
        Constructor<T, E>::copy(other._storage, _storage, ok_tag());
        _ok = true;
      } else {
        Constructor<T, E>::copy(other._storage, _storage, err_tag());
        _ok = false;
      }
    }
    ...
    typename std::enable_if<!std::is_same<U, void>::value, U>::type value()
        const {
      return _storage.template get<U>();
    }

    E error() const {
      return _storage.template get<E>();
    }

   private:
    bool _ok;
    Storage<T, E> _storage;
  };
